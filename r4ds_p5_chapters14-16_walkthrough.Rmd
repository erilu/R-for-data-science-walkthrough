...
Use the floor_date() function to group observations based on hour, then group by this parameter. Use this to calculate the average dep_delay per hour over the year. Plot using geom_line().

```{r}
flights_dt %>% 
  mutate ( floor_hour = floor_date(dep_time, "hour") )%>%
  group_by(floor_hour) %>%
  summarize(avg_delay_hour = mean(dep_delay, na.rm = T)) %>% 
  ggplot(aes(floor_hour, avg_delay_hour)) +
    geom_line()
```
