...
flights_dt %>% 
  ...
  ggplot(aes(dep_hour, avg_delay_hour)) +
    geom_line()
flights_dt %>% 
  mutate ( dep_hour = minute(dep_time) )%>%
  group_by(dep_hour) %>%
  summarize(avg_delay_hour = mean(dep_delay, na.rm = T)) %>% 
  ggplot(aes(dep_hour, avg_delay_hour)) +
    geom_line()
...
flights_dt %>% 
  ...
  group_by(wday) %>%
  summarize ( avg_delay_week = mean(dep_delay, na.rm = T))
```
