...
Let's first examine the distribution of each of these datasets, using histograms. We observe that there are no diamonds with a carat of 0, likewise there are no sched_dep_times with a value of 0. However the distribution of values between the two datasets is actually not very similar just by initial observation. The carat values are skewed to the right, but the sched_dep_time is not. I suppose one could argue that there are more flights with an earlier sched_dep_time, and likewise there are more diamonds with a low carat value.
