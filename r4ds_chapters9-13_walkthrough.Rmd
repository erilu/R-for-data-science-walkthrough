...
First join the planes table with the flights table. Then we can visualize any relationships between the year variable (age) of the plane and the delays associated with it (or group by tailnum, then caluclate avg delay, then join the info from planes, then visualize).

```{r}
# first join, then visualize
flights_planes <- left_join(flights, planes, by = "tailnum") %>%
  select ( tailnum, year.y, dep_delay, arr_delay)
flights_planes

# first group by tailnum, then join, then visualize
...
# dep_delay vs year of plane
avg_delay_plane %>%
  ggplot( aes ( year, avg_delay_dep )) +
    geom_point() +
    geom_smooth()
# arr_delay vs year of plane
avg_delay_plane %>%
...
Join the weather table with flights, and then we can visualize the relationship with delay and weather conditions.
