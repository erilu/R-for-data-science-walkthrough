...
If it contained weather records for all airports, it could be joined with flights using the primary key consisting of the columns year, month, day, hour and origin in ```weather```, which could be connected with the foreign key year, month, day, hour, and either origin or dest in ```flights```. For example,
```{r}
...
You could have a separate data frame containing the variables year, month, day, and "special", in which the "special" days would be designated TRUE and all other days in the data frame designated FALSE. This could be connected to the existing tables using the primary key consisting of year, month, and day. The value for the "special" column would be appended to the corresponding foriegn key values. For example, below is a table containing some special days of the year (Christmas, Thanksgiving, etc.), that is joined to the flights table.
